...
struct ColorDifference {
    ...
    /// - Returns: The color difference, or ΔE, between a sample color and a reference color.
    public static func difference(reference: CGColor, sample: CGColor, formula: ColorDistanceFormula = .CIE94) -> Double {
        let colorDifference: Double
        let refLab = reference.lab
        let smpLab = sample.lab
        switch formula {
        case .euclidean:
            colorDifference = Self.deltaEuclidean(reference: reference, sample: sample)
        case .CIE76:
            colorDifference = Self.deltaECIE76(reference: refLab, sample: smpLab)
        case .CIE94:
            colorDifference = Self.deltaECIE94(reference: refLab, sample: smpLab)
        case .CIEDE2000:
            colorDifference = Self.deltaCIEDE2000(reference: refLab, sample: smpLab)
        case .CMC:
            colorDifference = Self.deltaCMC(reference: refLab, sample: smpLab)
        }
        let roundedColorDifference = colorDifference.roundedDecimal(4)
        
        return roundedColorDifference
    }
    
    /// The color difference, or ΔE, between two colors according to the Euclidean formulas.
    ///
    /// https://en.wikipedia.org/wiki/Color_difference
    private static func deltaEuclidean(reference: CGColor, sample: CGColor) -> CGFloat {
        let deltaE = sqrt(pow(reference.red255 - sample.red255, 2) + pow(reference.green255 - sample.green255, 2) + pow(reference.blue255 - sample.blue255, 2))
        return deltaE
    }
    ...
    private static func deltaECIE76(reference: Lab, sample: Lab) -> CGFloat {
        let deltaE = sqrt(pow(sample.L - reference.L, 2) + pow(sample.a - reference.a, 2) + pow(sample.b - reference.b, 2))
        return deltaE
    }
    ...
    private static func deltaECIE94(reference: Lab, sample: Lab) -> CGFloat {
        let kL: CGFloat = 1.0
        let kC: CGFloat = 1.0
        let kH: CGFloat = 1.0
        ...
        let c1 = sqrt(pow(reference.a, 2) + pow(reference.b, 2))
        let sC = 1 + k1 * c1
        let sH = 1 + k2 * c1
        
        let deltaL = reference.L - sample.L
        let deltaA = reference.a - sample.a
        let deltaB = reference.b - sample.b
                
        let c2 = sqrt(pow(sample.a, 2) + pow(sample.b, 2))
        let deltaCab = c1 - c2
    ...
    private static func deltaCIEDE2000(reference: Lab, sample: Lab) -> CGFloat {
        /*
         ...
         */
        /* Equation 2 */
        let C1 = sqrt((reference.a * reference.a) + (reference.b * reference.b))
        let C2 = sqrt((sample.a * sample.a) + (sample.b * sample.b))
        /* Equation 3 */
        let barC = (C1 + C2) / 2.0
        /* Equation 4 */
        ...
        /* Equation 5 */
        let a1Prime = (1.0 + G) * reference.a
        let a2Prime = (1.0 + G) * sample.a
        /* Equation 6 */
        let CPrime1 = sqrt((a1Prime * a1Prime) + (reference.b * reference.b))
        let CPrime2 = sqrt((a2Prime * a2Prime) + (sample.b * sample.b))
        /* Equation 7 */
        var hPrime1: CGFloat
        if reference.b == 0 && a1Prime == 0 {
            hPrime1 = 0.0
        } else {
            hPrime1 = atan2(reference.b, a1Prime)
            /*
        ...
        var hPrime2: CGFloat
        if sample.b == 0 && a2Prime == 0 {
            hPrime2 = 0.0
        } else {
            hPrime2 = atan2(sample.b, a2Prime)
            /*
             * This must be converted to a hue angle in degrees between 0
             * and 360 by addition of 2􏰏 to negative hue angles.
             */
         ...
         */
        /* Equation 8 */
        let deltaLPrime = sample.L - reference.L
        /* Equation 9 */
         ...
         */
        /* Equation 12 */
        let barLPrime = (reference.L + sample.L) / 2.0
        /* Equation 13 */
    ...
    private static func deltaCMC(reference: Lab, sample: Lab) -> CGFloat {
        let l = 1.0
        let c = 1.0
        
        let lab1 = Lab(L: reference.L, a: reference.a, b: reference.b)
        let lab2 = Lab(L: sample.L, a: sample.a, b: sample.b)
